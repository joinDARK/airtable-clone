...
const UploadFiles: React.FC<Props> = ({editingHandler, typeCell, orderId, data}) => {
    ...
    try {
      ...
      if (res.status === 200) {
        if (res.data.message && res.data.message === "File not found") {
          ...
          setServerFiles(prev => prev.map(file => (file.id === editingFileId ? {...file, originalname: editingFileName} : file)))
          setEditingFileId(null)
          setEditingFileName("")
        }
      } else {
  ...
  return (
    <div className='space-y-4'>
      <form onSubmit={handleUpload}>
        <FileDragAndDropArea handleFileChange={handleFileChange} inputRef={inputRef} />
        {files.length > 0 && (
          <>
