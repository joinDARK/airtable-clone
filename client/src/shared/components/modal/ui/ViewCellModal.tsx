...
export default function ViewCellModal({value, type}: Props) {
    const data = useModalStore(item => item.formData)
    const typeCell = useModalStore(cellContent => cellContent.content)
    console.log(data);
    if (typeof value === 'string' && type == 'text') return <div>{value.length != 0 ? transformDate(value) : "—" }</div>
    else if (typeof value === 'number' && type == 'number') return <div>{value.toString()}</div>
    else if (Array.isArray(value) && type == 'related') return (
    ...
    else if (type == 'files' && typeCell == data?.files[0]?.type) return <div>
        <ServerFileList serverFiles={data?.files} />
    </div>
