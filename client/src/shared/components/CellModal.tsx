...
import {CellModalInputRenderer} from "../../modules/files/CellModalInputRenderer"
import {CellModalFileView} from "../../modules/files/CellModalFileView"
import {CellModalFileEditForm} from "../../modules/files/CellModalFileEditForm"
import {RelatedDataModal} from "../../modules/relationship/RelatedData"
import {reverseTransformDate} from "../../modules/date_formateer/dateFormateer"
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} setIsEditing={setIsEditing} isEditing={isEditing}>
      <div className='space-y-4'>
        {isEditing ? (
          column.type === "file" ? (
            <CellModalFileEditForm handleSave={handleSave} setIsEditing={setIsEditing} column={column} data={data} />
          ) : (
          ...
          <div className='space-y-4'>
            <div className='bg-gray-100 dark:bg-gray-700 p-4 rounded-lg'>
              {isRelationShip ? (
                ...
                <CellModalFileView data={data} column={column} handleDeleteFileById={handleDeleteFileById} handleSave={handleSave} />
              ) : Array.isArray(value) ? (
                <div>
                ...
                <div>{column.type === "boolean" ? (value === true ? "Да" : "Нет") : value}</div>
              )}
            </div>
          </div>
        )}
